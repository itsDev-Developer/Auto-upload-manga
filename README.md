...
postsettings - (ADMINS) Configure Post Channel / Dump Channel, manage posts
newpost - (ADMINS) Publish a new manga post (poster + title + genre + description + Read Now button)
```
...
<details>
    ...
    <code>POST_CHANNEL</code>: <b><i>[Optional] Default channel to publish manga posts to. Can also be set/changed at runtime with /postsettings.</i></b><br>
    <code>DUMP_CHANNEL</code>: <b><i>[Optional] Default channel used to store the files linked to a post. Can also be set/changed at runtime with /postsettings.</i></b><br>
</details>
...
<details>
    ...
    ▸ Post Manager — publish a poster + title + genre + description post with a "📖 Read Now" button; the linked file(s) are stored in your Dump Channel and delivered to the user by the bot on click (see <code>/newpost</code>, <code>/postsettings</code>)<br>
    </b>
</details>


### 5. **Post Manager (poster + title + genre + description + "Read Now")**
<details>
    <summary><b>Click Here</b></summary>
    <ol>
        <li>Run <code>/postsettings</code> (admin only) and set your <b>Dump Channel</b> (where files are stored) and <b>Post Channel</b> (where the formatted post is published). The bot must be an admin in both.</li>
        <li>Run <code>/newpost</code> (or tap "➕ Create New Post"). The bot will ask you to send the file(s) — they're saved straight into the Dump Channel — then the poster image, title, genre and description.</li>
        <li>Review the preview and tap "✅ Confirm & Publish". The post goes live in your Post Channel with a "📖 Read Now" button.</li>
        <li>When a user taps "📖 Read Now", the bot opens a private chat with them and sends the linked file(s) straight from the Dump Channel.</li>
        <li>Manage or delete existing posts any time from "📋 Manage Posts" inside <code>/postsettings</code>.</li>
    </ol>
</details>
